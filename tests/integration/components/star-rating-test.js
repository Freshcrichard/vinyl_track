...
test('Renders the full and empty stars correctly', function(assert) {
  ...
  assert.equal(this.$('.glyphicon-star').length, 4, 'The right amount of fullstars is rendered');
  ...
  assert.equal(this.$('.glyphicon-star').length, 2, 'The right amount of fullstars is rendered');
