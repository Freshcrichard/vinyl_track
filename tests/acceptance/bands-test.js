...
import moduleForAcceptance from 'vinyl_track/tests/helpers/module-for-acceptance';
...
test('List bands', function(assert) {
  ...
  andThen(function() {
    assertLength(assert, '.band-link', 2, 'All links are rendered');
...
test('Sort songs in various ways', function(assert){
  ...
  andThen(function(){
    ...
    assertTrimmedText(assert, '.song:last', 'New Fang', 'The last song is the lowerst ranked, last in the alphabet');
  });
...
test('Search songs', function(assert){
  ...
  andThen(function(){
    ...
    assertTrimmedText(assert, '.song:last', 'Mind Eraser, No Chaser', 'A matching songs that comes sooner in the alphabet appears at the bottom');
  });
});